...
2. **userStyles 內現有檔案的真實副檔名/格式。**
   `inspect` 會 pull 一個現有檔到本機讓我們確認命名規則與副檔名。`push` 的過濾副檔名做成 `--ext`（預設先留空，inspect 跑完才填真實值，例如 `.xmp`）。
...
- 旗標：
  - `--source string`：本機來源，**可為單一檔案或資料夾**。
  - `--ext string`：過濾副檔名（預設空＝不過濾；inspect 後填真實值）。
...
**副檔名過濾：** 若 `--ext` 非空，只推符合的檔（資料夾遞迴時逐檔判斷）。
...
4. 依資料夾 mtime 取**最新**一個為採用值。
5. `target = {latest}/settings-acr/userStyles`。
6. 多候選時印出完整清單，並明確標示採用哪一個。
7. 找不到任何候選 → 明確報錯，提示可能 app 尚未產生 catalog 或路徑前綴需手動指定。
...
- **里程碑 1（首交付）：** 可編譯可跑的 `inspect`（含 device/afcfs/locate 基礎）。在使用者實機跑一次，取得真實 AFC root 前綴與副檔名。
