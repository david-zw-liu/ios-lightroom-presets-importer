...
- `internal/pushsync/plan.go` — `PlanPush(source, userStylesDir, ext) (Plan, error)` pure local-fs planning.
...
3. The real extension/format of the pulled sample in `./_inspect_sample/`.
Update `docs/superpowers/specs/2026-06-30-lrpush-design.md` §2 with the observed `--ext` default and any path-prefix finding before starting Milestone 2.
...
- Produces:
  ...
  - `func PlanPush(source, userStylesDir, ext string) (Plan, error)` — single file → one op at `userStylesDir/<base>`; directory → `ReplaceDir = userStylesDir/<base(source)>`, ops for every file under source (filtered by ext, case-insensitive), device path preserves relative subdirs under ReplaceDir. Source must exist.
...
func TestPlanPushSingleFile(t *testing.T) {
	...
	plan, err := PlanPush(src, "Documents/123/userStyles", ".xmp")
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlanPushDirPreservesStructureAndBasename(t *testing.T) {
	...
	writeFile(t, filepath.Join(src, "note.txt")) // filtered out by ext
...
func TestPlanPushNoExtFilter(t *testing.T) {
	dir := t.TempDir()
	...
	if len(plan.Ops) != 2 {
		t.Fatalf("want 2 ops without filter, got %+v", plan.Ops)
	}
}
...
func extMatch(name, ext string) bool {
	if ext == "" {
		return true
	}
...
func PlanPush(source, userStylesDir, ext string) (Plan, error) {
...
git commit -m "feat: add push planning (file/dir mapping, basename layer, ext filter)"
```
...
func TestExecuteDryRunDoesNotMutate(t *testing.T) {
	...
	plan, _ := PlanPush(filepath.Join(dir, "my-presets"), "U", ".xmp")
...
- Produces: `func warningBanner() string`; `func newPushCmd() *cobra.Command` with flags `--source` (required), `--ext`, `--backup-dir`, `--commit`, `--catalog`.
...
func newPushCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			plan, err := pushsync.PlanPush(source, chosen.UserStyles, ext)
			if err != nil {
				return err
			}
			if backupDir == "" {
	...
	cmd.Flags().StringVar(&ext, "ext", "", "only push files with this extension (e.g. .xmp); empty = all")
...
1. `go run ./cmd/lrpush push --source ./testpresets --ext <observed-ext>` (dry-run) — confirm target, backup, and file list look right.
2. `go run ./cmd/lrpush push --source ./testpresets --ext <observed-ext> --commit` — confirm backup created under `./_userStyles_backup/`, OK lines per file.
...
Create `README.md` with these sections (fill the `--ext` default with the value observed during the Task 5 manual inspect):
```markdown
# lrpush
...
backed up first). Other existing userStyles content is left untouched.
    ...
    # preview
    ./lrpush push --source ./my-presets --ext .xmp
    # apply
    ./lrpush push --source ./my-presets --ext .xmp --commit

Single file:

    ./lrpush push --source ./foo.xmp --ext .xmp --commit
...
- `--ext` — extension filter for push (e.g. `.xmp`)
...
- **Spec coverage:** device resolution + `--udid` (Task 3); house_arrest + `--bundle-id` (Tasks 1/3); path selection incl. settings-acr filter + multi-candidate menu + `--path-prefix`/`--catalog` (Task 4); backup before push (Task 7); ext-filtered push (Tasks 6-8); dry-run default + `--commit` (Tasks 7-10); warning banner (Task 8); per-file logging (Tasks 7/9); folder-layer preservation + recursive structure (Task 6); merge semantics / wholesale folder replace (Tasks 6-7); rm multiple relative paths (Tasks 9-10); README (Task 11); inspect tree dump + sample pull (Task 5). All covered.
